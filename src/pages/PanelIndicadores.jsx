...
export default function PanelIndicadores() {
    ...
    (async () => {
      ...
      try {
        const data = await api.get(
          "/licitaciones/with-fields?fields=id,id_licitacion,nombre_entidad,rut_entidad,fecha,fecha_adjudicada,estado,creado_por,total_con_iva,total_sin_iva,monto,tipo_compra,tipo_cliente"
        );
        const rows = data || [];
  ...
  function clientesNuevosYRecompra(mesKey) {
    ...
    Object.values(clienteAdjMeses).forEach((meses) => {
      const enMes = meses.some((mk) => mk === mesKey);
      if (!enMes) return;
      totalConCompra++;
      const primera = meses.reduce((a, b) => (a < b ? a : b));
      if (primera === mesKey) nuevos++;
      const tuvoAntes = meses.some((mk) => mk < mesKey);
      if (tuvoAntes) repiten++;
    });
  ...
  const cnr = useMemo(() => clientesNuevosYRecompra(mesActual), [clienteAdjMeses, mesActual]); // eslint-disable-line
  const cnrPrev = useMemo(() => clientesNuevosYRecompra(mesPrev), [clienteAdjMeses, mesPrev]); // eslint-disable-line
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 16, marginBottom: 16 }}>
            ...
            {/* Embudo */}
            <div className="surface" style={{ padding: 18 }}>
              <h3 className="surface-title" style={{ marginBottom: 14 }}>Embudo de conversión (mes)</h3>
