...
import CorreosCotizacionSection from "../components/CorreosCotizacionSection";
import { GripVertical, Plus, Trash2, MessageSquare, Minus, ChevronUp, ChevronDown, Mail } from "lucide-react";
...
export default function EditarLicitacion() {
  ...
  const [drawerComunicacionesOpen, setDrawerComunicacionesOpen] = useState(false);
...
============================================================ */
  ...
  async function subirDocumento() {
    ...
    try {
      ...
      // Si era la PRIMERA orden de compra de esta cotización, disparar correo de adjudicación.
      // El backend valida idempotencia (correo_adjudicacion_enviado_at) así que es seguro
      // llamar siempre — pero si podemos saber acá, evitamos un round-trip extra.
      if (tipo === "orden_compra") {
        const ocsActuales = (documentos || []).filter((d) => d.tipo === "orden_compra").length;
        if (ocsActuales === 0) {
          try {
            await api.post(`/cotizaciones/${Number(id)}/comunicaciones/trigger`, {
              trigger: "adjudicacion_oc",
            });
          } catch (errTrigger) {
            console.warn("Trigger adjudicación no se pudo enviar:", errTrigger);
            // No bloqueamos el upload por esto.
          }
        }
      }
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <div className="btn-row">
          <button
            type="button"
            onClick={() => setDrawerComunicacionesOpen(true)}
            className="btn btn-secondary"
            title="Ver y enviar correos al cliente"
            style={{ display: "inline-flex", alignItems: "center", gap: 6 }}
          >
            <Mail size={14} /> Comunicaciones
          </button>
      ...
      <CorreosCotizacionSection
        open={drawerComunicacionesOpen}
        onClose={() => setDrawerComunicacionesOpen(false)}
        licitacion={{
          id: Number(id),
          id_licitacion: idLicitacionInput,
          nombre_entidad: nombreEntidad,
