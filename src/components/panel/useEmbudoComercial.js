...
export default function useEmbudoComercial(mesActualKey) {
  ...
  const logrosPorCliente = useMemo(() => {
    ...
    // 1) Primera fecha de cada acción por cliente particular.
    const base = new Map(); // cid -> { pDate, cDate, zDate, mDate }
    const ensure = (cid) => {
      let e = base.get(cid);
      if (!e) { e = { pDate: null, cDate: null, zDate: null, mDate: null }; base.set(cid, e); }
      return e;
    };
    actividades.forEach((a) => {
      ...
      if (!f) return;
      const e = ensure(cid);
      if (MOTIVOS_PROSPECTO.includes(a.motivo)) e.pDate = minDate(e.pDate, f);
      if (TIPOS_CONTACTO.includes(a.tipo)) e.cDate = minDate(e.cDate, f);
      if (a.motivo === MOTIVO_COTIZA) e.zDate = minDate(e.zDate, f);
    });
    ...
    actividades.forEach((a) => {
      ...
      if (lic && lic.adjudicada && lic.adjDate) ensure(cid).mDate = minDate(ensure(cid).mDate, lic.adjDate);
    });
    ...
    base.forEach((e) => {
      ...
      arr.push({ achP, achC, achZ, achM });
    });
    return arr;
  }, [actividades, licById, particularIds]);
  ...
  return { loading: loading && puedeVer, puedeVer, emb, embPrev, evolucion, conv, maxEmbudo };
}
